...
const recruiterSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchDeleteRecruiterById.pending, (state) => {
      state.isLoading = true;
    });
    builder.addCase(fetchDeleteRecruiterById.fulfilled, (state) => {
      state.isLoading = false;
    });
    builder.addCase(fetchDeleteRecruiterById.rejected, (state, action) => {
      state.isLoading = false;
      state.error = action.payload as string;
    });
  },
